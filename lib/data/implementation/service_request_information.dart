import 'package:service_la/services/api_constants/api_const.dart';
import 'package:service_la/services/api_service/api_service.dart';
import 'package:service_la/data/network/service_request_api_service.dart';

class ServiceRequestInformation extends ServiceRequestApiService {
  @override
  Future postServiceRequests(params) async {
    dynamic response = await ApiService().post(ApiConstant.postServiceRequestsPath, params);
    return response;
  }

  @override
  Future getServiceRequestsDetails(String serviceId) async {
    dynamic response = await ApiService().get(ApiConstant.getServiceRequestsDetailsPath.replaceAll("#serviceId#", serviceId));
    return response;
  }

  @override
  Future postServiceRequestBids(params) async {
    dynamic response = await ApiService().post(ApiConstant.postServiceRequestBidsPath, params);
    return response;
  }

  @override
  Future putServiceRequestBids(bidId, params) async {
    dynamic response = await ApiService().put(ApiConstant.putServiceRequestBidsPath.replaceAll("#bidId#", bidId), params);
    return response;
  }

  @override
  Future putServiceRequestBidsShortlist(bidId, params) async {
    dynamic response = await ApiService().put(ApiConstant.putServiceRequestBidsShortlistPath.replaceAll("#bidId#", bidId), params);
    return response;
  }

  @override
  Future putServiceRequestBidsApproval(bidId, params) async {
    dynamic response = await ApiService().put(ApiConstant.putServiceRequestBidsApprovalPath.replaceAll("#bidId#", bidId), params);
    return response;
  }

  @override
  Future putServiceRequestBidsStatus(bidId, params) async {
    dynamic response = await ApiService().put(ApiConstant.putServiceRequestBidsStatusPath.replaceAll("#bidId#", bidId), params);
    return response;
  }

  @override
  Future putServiceRequestsStatus(serviceId, params) async {
    dynamic response = await ApiService().put(ApiConstant.putServiceRequestsStatusPath.replaceAll("#serviceId#", serviceId), params);
    return response;
  }
}
